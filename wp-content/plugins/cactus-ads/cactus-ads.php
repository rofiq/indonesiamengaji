	...
	*/
	...
	function cactus_ads_shortcode_stand_alone($atts, $content = "")
	{
		...
		if($enable_ads != '' && $enable_ads == 'yes')
		{
			...
			{
				global $wpdb;
				$adv = $wpdb->get_results("select id from $wpdb->posts inner join $wpdb->postmeta on " . $wpdb->prefix ."posts.id = $wpdb->postmeta.post_id where post_status='publish' and post_type='video-advs' and $wpdb->postmeta.meta_key = 'advs_expiry_date' and $wpdb->postmeta.meta_value >= NOW() order by rand() limit 1");
