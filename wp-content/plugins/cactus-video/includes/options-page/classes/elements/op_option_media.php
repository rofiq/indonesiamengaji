...
if(!class_exists('OP_Option_media')){
	class OP_Option_media extends OP_Option{
		...
		public function getOption($selectedValue = null){
			...
			$html = '<input type="text" name="'.$this->name.'" value="'.$selectedValue.'"/> <a class="button upload_logo" href="#">' . esc_html__('Upload Media','osp').'</a> <div class="logo_image_holder">' . ($selectedValue == '' ? '' : '<img src="'.$selectedValue.'"/>').'</div>';
